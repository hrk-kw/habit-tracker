import { getExercisesByCategory, saveGymSession } from './db.js';
...
function lastLabel(exercise) {
  if (exercise.default_weight_kg == null || exercise.default_reps == null) return '前回なし';
...
function updateBadge(exerciseId) {
  const badgeEl = document.querySelector(`.exercise-count-badge[data-exercise-id="${exerciseId}"]`);
  if (!badgeEl) return;
...
function applyEditorValue() {
  ...
  pendingSets.set(editingExerciseId, {
    weightKg: Number.isFinite(weightKg) ? weightKg : currentEffective.weightKg,
...
async function save() {
  ...
  await saveGymSession(date, entries);
  await badge.refresh();
  location.hash = '#home';
}

export async function render() {
  exercises = await getExercisesByCategory('outer');
  pendingSets = new Map();
  ...
  document.getElementById('gym-save-btn').onclick = save;
